...
/* 要写入到24c02的字符串数组 */
const uint8_t g_text_buf[] = {"STM32 IIC TEST HJX"};
...
int main(void)
{
    ...
    lcd_show_string(30, 70, 200, 16, 16, "IIC TEST HJX", RED);
    ...
    while (1)
    {
        ...
        if (key == KEY1_PRES)   /* KEY1按下,写入24C02 */
        {
            ...
            //at24cxx_pagewrite(0, (uint8_t *)g_text_buf, TEXT_SIZE);
            at24cxx_write(0, (uint8_t *)g_text_buf, TEXT_SIZE);  // 按字节写
            //at24cxx_bufwrite(0, (uint8_t *)g_text_buf, TEXT_SIZE);
        ...
        if (key == KEY0_PRES)   /* KEY0按下,读取字符串并显示 */
        {
            ...
            //at24cxx_sequentialread(0, datatemp, TEXT_SIZE);
            at24cxx_read(0, datatemp, TEXT_SIZE);  // 按字节读
